...
The `loans` table is the single source of truth for all downstream analytics and ML. It exists in two forms: as a PostgreSQL table with strict CHECK constraints enforcing domain integrity, and as a Hive external table partitioned by `grade` and bucketed by `id`.
...
- **Text block** - describes the grid search setup: 3-fold cross-validation, `areaUnderPR` as the optimization metric, a 27-combination parameter grid per model, and the winning hyperparameters found for each model.
...
2. **Temporal non-stationarity** — Default rates vary from 14% to 26% across issue years, and the 2018 cohort appears artificially safe because recently issued loans have not had enough time to charge off. Because loan records are independent, a random split does not introduce strict data leakage; however, it does mean the test set may not represent the temporal distribution of a real production setting, where the model is always applied to loans issued after its training cutoff.
...
5. **Data quality issues** — A small number of records contained DTI values of −1 and 999, which are sentinel codes rather than real measurements. These required explicit range filters (`dti >= 0 AND dti <= 100`) in EDA queries to avoid distorting aggregate statistics. More broadly, many of the 151 raw columns had high sparsity, which informed the decision to restrict the analysis to the 26 most complete and interpretable columns.
6. **Cluster resource limits** — YARN executor memory constraints on the university cluster limited the grid search to 27 parameter combinations per model and k = 3 cross-validation folds. Attempts to use larger grids or more folds resulted in out-of-memory failures, forcing the hyperparameter search space to remain narrower than ideal.
...
| Hyperparameter tuning     | Run grid search (27 combinations per model, 3-fold CV, PR-AUC optimisation) for all three model families, save best models to HDFS, export predictions and evaluation metrics                                                                                                  | 0%             | 100%               | 0%           | 0%                    | `models/model1` – `models/model3`, `output/dashboard/evaluation.csv`, `output/dashboard/prediction_distribution.csv`, `scripts/train_tuned_models.py`  | 3                |
